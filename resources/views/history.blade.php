                ...
                <div class="bg-gradient-to-br from-[#F6A406] to-[#F08C00] rounded-2xl p-8 text-white shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-4xl font-bold">{{ $stats['total_orders'] }}</h3>
                        </div>
                ...
                <div class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-8 text-white shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-2xl font-bold">Rp {{ number_format($stats['total_spent']) }}</h3>
                        </div>
                        <div class="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl p-8 text-white shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-4xl font-bold">{{ $stats['completed_orders'] }}</h3>
                        </div>
                        <div class="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                ...
                <form method="GET" action="/history" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Nomor order atau nama produk..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="sort_by" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                                ...
                                <option value="order_number" {{ request('sort_by') == 'order_number' ? 'selected' : '' }}>Nomor Order</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="date_from" value="{{ request('date_from') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                        </div>

                        <!-- Tanggal Sampai -->
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Sampai Tanggal</label>
                            <input type="date" name="date_to" value="{{ request('date_to') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                        </div>
                    </div>
                ...
                @if(request()->hasAny(['search', 'status', 'date_from', 'date_to']))
                    <div class="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 mb-6 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-blue-800">Menampilkan <strong>{{ $orders->total() }}</strong> hasil</span>
                        </div>
                    </div>
                @endif
                    ...
                    @foreach($orders as $order)
                        <div class="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-8 border border-orange-100">
                            <div class="flex justify-between items-start mb-6">
                                <div>
                                    <h3 class="text-lg font-bold text-black mb-1">{{ $order->order_number }}</h3>
                                    <p class="text-sm text-gray-500">{{ $order->created_at?->timezone('Asia/Jakarta')->format('d M Y, H:i') }}</p>
                                </div>
                                <div class="text-right">
                                    ...
                                    @endphp
                                    <span class="inline-block px-3 py-1 text-xs font-semibold rounded-full {{ $config['bg'] }} {{ $config['text'] }}">
                                        {{ $config['label'] }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="mb-6 bg-gradient-to-r from-orange-50 to-yellow-50 rounded-xl p-6">
                                ...
                                <div class="flex items-center justify-between relative">
                                    ...
                                    <div class="absolute top-5 left-0 right-0 h-1 bg-gray-200 -z-0">
                                        ...
                                        @endphp
                                        <div class="h-full bg-gradient-to-r from-[#F6A406] to-[#F08C00] transition-all duration-500" style="width: {{ $progress }}"></div>
                                    </div>

                                    <!-- Step 1: Pending -->
                                    <div class="flex flex-col items-center z-10">
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center {{ in_array($order->status, ['pending', 'cooking', 'delivering', 'completed']) ? 'bg-gradient-to-br from-[#F6A406] to-[#F08C00] text-white' : 'bg-gray-200 text-gray-400' }} shadow-lg">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                        </div>
                                        <span class="text-xs font-semibold mt-2 {{ in_array($order->status, ['pending', 'cooking', 'delivering', 'completed']) ? 'text-[#F6A406]' : 'text-gray-400' }}">Diterima</span>
                                    </div>

                                    <!-- Step 2: Cooking -->
                                    <div class="flex flex-col items-center z-10">
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center {{ in_array($order->status, ['cooking', 'delivering', 'completed']) ? 'bg-gradient-to-br from-[#F6A406] to-[#F08C00] text-white' : 'bg-gray-200 text-gray-400' }} shadow-lg">
                                            <span class="text-lg">👨‍🍳</span>
                                        </div>
                                        <span class="text-xs font-semibold mt-2 {{ in_array($order->status, ['cooking', 'delivering', 'completed']) ? 'text-[#F6A406]' : 'text-gray-400' }}">Dimasak</span>
                                    </div>

                                    <!-- Step 3: Delivering -->
                                    <div class="flex flex-col items-center z-10">
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center {{ in_array($order->status, ['delivering', 'completed']) ? 'bg-gradient-to-br from-[#F6A406] to-[#F08C00] text-white' : 'bg-gray-200 text-gray-400' }} shadow-lg">
                                            <span class="text-lg">🚚</span>
                                        </div>
                                        <span class="text-xs font-semibold mt-2 {{ in_array($order->status, ['delivering', 'completed']) ? 'text-[#F6A406]' : 'text-gray-400' }}">Diantar</span>
                                    </div>

                                    <!-- Step 4: Completed -->
                                    <div class="flex flex-col items-center z-10">
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center {{ $order->status === 'completed' ? 'bg-gradient-to-br from-green-500 to-emerald-600 text-white' : 'bg-gray-200 text-gray-400' }} shadow-lg">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                            </svg>
                                        </div>
                                        <span class="text-xs font-semibold mt-2 {{ $order->status === 'completed' ? 'text-green-600' : 'text-gray-400' }}">Selesai</span>
                                    </div>
                                </div>
                            </div>
                            @endif
                                ...
                                @foreach($order->items as $item)
                                    <div class="flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h4 class="font-semibold text-black">{{ $item->product_name }}</h4>
                                                <p class="text-sm text-gray-500">{{ $item->quantity }} x Rp. {{ number_format($item->product_price) }}</p>
                                            </div>
                                        </div>
                                        <div class="text-right font-semibold text-black">Rp. {{ number_format($item->total) }}</div>
                                    </div>
                                @endforeach
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600 mb-4">
                                <div>
                                    ...
                                    <span class="block capitalize">{{ $order->payment_method }}</span>
                                </div>
                                <div>
                                    <span class="block font-semibold text-gray-900">Alamat</span>
                                    <span class="block">{{ $order->customer_address }}</span>
                                </div>
                                <div>
                                ...
                                <div class="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl px-6 py-4 flex items-center justify-between mt-4">
                                    <div class="flex items-center gap-4">
                                        <div class="flex items-center gap-1 text-2xl">
                                            @for($i = 0; $i < $order->feedback_rating; $i++)
                                                <span class="text-yellow-400">★</span>
                                            @endfor
                                        </div>
                                        <span class="text-sm text-gray-700 font-medium">{{ $order->feedback_comment ?? 'Terima kasih atas pesanan Anda.' }}</span>
                                    </div>
                                    <span class="text-xs text-gray-500 bg-white px-3 py-1 rounded-full">{{ $order->feedback_submitted_at?->diffForHumans() }}</span>
                                </div>
                            @endif

                            <!-- Tombol Konfirmasi untuk Pesanan Selesai -->
                            @if($order->status === 'completed' && !$order->is_confirmed)
                                <div class="mt-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-2xl p-6">
                                    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                                        ...
                                        <div class="flex gap-3 w-full md:w-auto">
                                            <form action="{{ route('history.confirm', $order->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin pesanan sudah diterima dengan baik?')" class="flex-1 md:flex-none">
                                                @csrf
                                            ...
                                            <button onclick="openContactModal('{{ $order->order_number }}')" class="flex-1 md:flex-none bg-gradient-to-r from-red-500 to-rose-600 text-white px-6 py-3 rounded-xl font-bold hover:shadow-2xl transition-all duration-300 hover:scale-105 flex items-center justify-center gap-2">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            @elseif($order->status === 'completed' && $order->is_confirmed)
                                <div class="mt-6 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-2xl p-6">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-sm text-green-700">Dikonfirmasi pada {{ $order->confirmed_at?->timezone('Asia/Jakarta')->format('d M Y, H:i') }}</p>
                                        </div>
                                        <div class="bg-white px-4 py-2 rounded-xl shadow-md">
                                            <span class="text-xs text-gray-500">{{ $order->confirmed_at?->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                </div>
                            @endif
