...
@section('title', 'Edit Produk - FoodMate Admin')
...
<div class="max-w-4xl mx-auto">
    ...
    <!-- Form -->
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            @if($product->image_path)
                <div>
                    ...
                    <img src="{{ asset($product->image_path) }}" alt="{{ $product->name }}" class="w-32 h-32 object-cover rounded-lg">
                </div>
            @endif
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                @error('name')
            ...
            <div>
                ...
                <textarea name="description" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="price" value="{{ old('price', $product->price) }}" required min="0" step="1000" class="w-full pl-12 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                </div>
                @error('price')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="rating" value="{{ old('rating', $product->rating) }}" min="0" max="5" step="0.1" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                @error('rating')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="use_stock_system" id="use_stock_system" value="1" {{ old('use_stock_system', $product->use_stock_system) ? 'checked' : '' }} class="w-5 h-5 text-[#F6A406] border-gray-300 rounded focus:ring-[#F6A406]">
                        <label for="use_stock_system" class="text-sm font-semibold text-gray-700">Gunakan Sistem Stok Harian</label>
                    </div>

                    <!-- Stock Fields -->
                    <div id="stock_fields" class="space-y-4 {{ old('use_stock_system', $product->use_stock_system) ? '' : 'hidden' }}">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Stok Harian Default</label>
                            <input type="number" name="default_daily_stock" value="{{ old('default_daily_stock', $product->default_daily_stock) }}" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Minimum Stok (Alert)</label>
                            <input type="number" name="minimum_stock" value="{{ old('minimum_stock', $product->minimum_stock) }}" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 pt-6">
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="w-5 h-5 text-[#F6A406] border-gray-300 rounded focus:ring-[#F6A406]">
...
<script>
    ...
    newCategoryInput.addEventListener('blur', function() {
        if (!this.value) {
            ...
            this.name = '';
            this.required = false;
            categorySelect.value = '{{ old('category', $product->category) }}';
        }
    });
