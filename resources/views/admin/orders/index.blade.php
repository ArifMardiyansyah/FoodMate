...
@section('title', 'Manajemen Pesanan - FoodMate Admin')
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-4 transition-all duration-200 hover:shadow-xl hover:scale-105">
            ...
            <p class="text-2xl font-bold text-orange-600">{{ $stats['cooking'] }}</p>
        </div>
        <div class="bg-white rounded-2xl shadow-lg p-4 transition-all duration-200 hover:shadow-xl hover:scale-105">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-4 transition-all duration-200 hover:shadow-xl hover:scale-105">
            ...
            <p class="text-lg font-bold text-green-600">Rp {{ number_format($stats['total_revenue'] / 1000) }}K</p>
        </div>
    </div>
        ...
        <form method="GET" action="{{ route('admin.orders.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama makanan..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                </div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('admin.orders.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-xl font-semibold hover:bg-gray-300 transition-colors">
                        Reset
                    </a>
                </div>
            </div>
                    ...
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                @endphp
                                <span class="inline-block px-3 py-1 text-xs font-semibold rounded-full {{ $paymentColors[$order->payment_status] ?? 'bg-gray-100 text-gray-800' }}">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    @if($order->payment_status === 'pending')
                                        <button onclick="confirmPayment({{ $order->id }})" class="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition-colors" title="Konfirmasi Pembayaran">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                        </button>
                                    @endif
                                    <a href="{{ route('admin.orders.show', $order->id) }}" class="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition-colors" title="Lihat Detail">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    </a>
                                    <button onclick="openStatusModal({{ $order->id }}, '{{ $order->status }}')" class="p-2 bg-orange-100 text-orange-600 rounded-lg hover:bg-orange-200 transition-colors" title="Update Status">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
