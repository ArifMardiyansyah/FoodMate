...
@section('title', 'Dashboard Admin - FoodMate')
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-8 bg-gradient-to-r from-[#F6A406] to-[#F08C00] rounded-3xl p-8 shadow-2xl">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-white/90 mt-2 text-lg">Selamat datang kembali, {{ auth()->user()->name }}! 👋</p>
            </div>
            <div class="hidden md:block">
                <div class="bg-white/20 backdrop-blur-sm rounded-2xl p-4 border-2 border-white/30">
                    ...
                    <p class="text-white font-bold text-xl">{{ \Carbon\Carbon::now()->format('d M Y') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl p-8 mb-8 border border-gray-100">
        ...
        <form method="GET" action="{{ route('admin.dashboard') }}" class="flex flex-wrap gap-4 items-end">
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="date" name="date_from" value="{{ $dateFrom }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
            </div>
            <div class="flex-1 min-w-[200px]">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Sampai Tanggal</label>
                <input type="date" name="date_to" value="{{ $dateTo }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-3xl shadow-2xl p-8 text-white transition-all duration-300 hover:shadow-2xl hover:scale-105 border-2 border-blue-400/30 relative overflow-hidden">
            ...
            <h3 class="text-4xl font-bold mb-2 relative z-10">{{ $todayStats['orders'] }}</h3>
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-3xl shadow-2xl p-8 text-white transition-all duration-300 hover:shadow-2xl hover:scale-105 border-2 border-green-400/30 relative overflow-hidden">
            ...
            <h3 class="text-4xl font-bold mb-2 relative z-10">Rp {{ number_format($todayStats['revenue']) }}</h3>
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-3xl shadow-2xl p-8 text-white transition-all duration-300 hover:shadow-2xl hover:scale-105 border-2 border-purple-400/30 relative overflow-hidden">
            ...
            <h3 class="text-4xl font-bold mb-2 relative z-10">{{ $todayStats['customers'] }}</h3>
                ...
                @forelse($topProducts as $index => $product)
                    <div class="flex items-center gap-4">
                        ...
                        <div class="text-right">
                            <p class="font-bold text-green-600">Rp {{ number_format($product->total_revenue) }}</p>
                        </div>
                    </div>
                @empty
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('admin.orders.index') }}" class="text-[#F6A406] hover:text-[#F08C00] font-semibold text-sm">
                    Lihat Semua →
                </a>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($recentOrders as $order)
                            <tr class="hover:bg-gray-50">
                                <td class="py-3 px-4">
                                    <a href="{{ route('admin.orders.show', $order->id) }}" class="text-[#F6A406] hover:underline font-semibold">
                                        {{ $order->order_number }}
                                    </a>
                                </td>
                                <td class="py-3 px-4 text-gray-900">{{ $order->customer_name }}</td>
                                ...
                                <td class="py-3 px-4 font-semibold text-gray-900">Rp {{ number_format($order->total) }}</td>
                                <td class="py-3 px-4">
                                    @php
                                        $statusColors = [
        ...
        <div class="space-y-6">
            <div class="bg-gradient-to-br from-red-50 to-orange-50 rounded-3xl shadow-xl p-8 border-2 border-red-200">
                ...
                <div class="space-y-3">
                    @forelse($lowStockProducts->take(5) as $stock)
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="font-semibold text-gray-900 text-sm">{{ $stock->product->name }}</p>
                                <p class="text-xs text-gray-500">Sisa: {{ $stock->current_stock }}</p>
                            </div>
                            <a href="{{ route('admin.stock.index') }}" class="text-[#F6A406] hover:text-[#F08C00] text-sm font-semibold">
                                Update
                            </a>
                        </div>
                    @empty
            ...
            <div class="bg-gradient-to-br from-blue-50 to-purple-50 rounded-3xl shadow-xl p-8 border-2 border-blue-200">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-bold text-gray-900">{{ $overallStats['total_orders'] }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600">Total Revenue</span>
                        <span class="font-bold text-green-600">Rp {{ number_format($overallStats['total_revenue']) }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600">Total Customer</span>
                        <span class="font-bold text-gray-900">{{ $overallStats['total_customers'] }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600">Total Produk</span>
                        <span class="font-bold text-gray-900">{{ $overallStats['total_products'] }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600">Rating Rata-rata</span>
                        <span class="font-bold text-yellow-600">{{ number_format($avgRating, 1) }} ⭐</span>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    new Chart(salesTrendCtx, {
        type: 'line',
        data: {
            labels: {!! json_encode($salesTrend->pluck('date')->map(fn($d) => \Carbon\Carbon::parse($d)->format('d M'))) !!},
            datasets: [{
                label: 'Revenue (Rp)',
    ...
    new Chart(orderStatusCtx, {
        type: 'doughnut',
        data: {
            labels: {!! json_encode($ordersByStatus->pluck('status')->map(fn($s) => ucfirst($s))) !!},
            datasets: [{
    ...
    new Chart(paymentMethodCtx, {
        type: 'bar',
        data: {
            labels: {!! json_encode($paymentMethods->pluck('payment_method')->map(fn($p) => ucfirst($p))) !!},
            datasets: [{
