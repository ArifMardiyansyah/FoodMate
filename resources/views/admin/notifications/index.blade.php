...
@section('title', 'Notifikasi Pesanan - FoodMate Admin')
...
<div class="max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="hidden md:block">
                <div class="bg-white/20 backdrop-blur-sm rounded-2xl p-4 border-2 border-white/30">
                    ...
                    <p class="text-white font-bold text-3xl">{{ $stats['total'] }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl shadow-xl p-6 text-white transition-all duration-200 hover:shadow-2xl hover:scale-105">
            ...
            <h3 class="text-3xl font-bold mb-1">{{ $stats['unread'] }}</h3>
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl shadow-xl p-6 text-white transition-all duration-200 hover:shadow-2xl hover:scale-105">
            ...
            <h3 class="text-3xl font-bold mb-1">{{ $stats['read'] }}</h3>
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl shadow-xl p-6 text-white transition-all duration-200 hover:shadow-2xl hover:scale-105">
            ...
            <h3 class="text-3xl font-bold mb-1">{{ $stats['today'] }}</h3>
        ...
        <form method="GET" action="{{ route('admin.notifications.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari notifikasi..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#F6A406] focus:border-transparent">
                </div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('admin.notifications.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-xl font-semibold hover:bg-gray-300 transition-colors">
                        Reset
                    </a>
                </div>
            </div>
    ...
    <div class="space-y-4">
        @forelse($notifications as $notification)
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-200 hover:shadow-xl {{ !$notification->is_read ? 'border-l-4 border-[#F6A406]' : '' }}">
                <div class="p-6">
                    <div class="flex items-start justify-between gap-4">
                                    ...
                                    <div class="bg-gray-50 rounded-xl p-4 mb-3">
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                                            <div>
                                                ...
                                                <p class="font-semibold text-gray-900">{{ $notification->order->order_number }}</p>
                                            </div>
                                            <div>
                                                <p class="text-gray-500 mb-1">Pelanggan</p>
                                                <p class="font-semibold text-gray-900">{{ $notification->order->customer_name }}</p>
                                            </div>
                                            <div>
                                                <p class="text-gray-500 mb-1">Total</p>
                                                <p class="font-semibold text-green-600">Rp {{ number_format($notification->order->total) }}</p>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <div class="flex flex-col gap-2">
                            @if($notification->order)
                                <a href="{{ route('admin.orders.show', $notification->order->id) }}" class="bg-blue-500 text-white px-4 py-2 rounded-xl font-semibold hover:bg-blue-600 transition-colors shadow-md hover:shadow-lg text-sm text-center">
                                    Lihat Pesanan
                                </a>
                            @endif
                            
                            @if(!$notification->is_read)
                                <button onclick="markAsRead({{ $notification->id }})" class="bg-green-500 text-white px-4 py-2 rounded-xl font-semibold hover:bg-green-600 transition-colors shadow-md hover:shadow-lg text-sm">
                                    Tandai Dibaca
                                </button>
                            @endif

                            <form action="{{ route('admin.notifications.delete', $notification->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus notifikasi ini?')">
                                @csrf
                                @method('DELETE')
