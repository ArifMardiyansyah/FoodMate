    ...
    <div class="max-w-4xl mx-auto px-6 py-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="/cart" class="relative inline-flex items-center justify-center w-12 h-12 bg-[#fff7ed] rounded-full hover:bg-[#ffedd5] transition">
                ...
                <span class="absolute -top-1 -right-1 min-w-[20px] h-5 bg-[#f97316] rounded-full text-xs font-semibold text-white flex items-center justify-center px-1">{{ number_format($cartCount) }}</span>
            </a>
        </div>
        ...
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                @foreach($favorites as $product)
                    <a href="{{ route('menu.show', $product->slug) }}" class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-xl transition-shadow">
                        <div class="relative h-48 bg-gray-100">
                            ...
                            <div class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-full flex items-center gap-1.5">
                                ...
                                <span class="text-xs font-semibold text-gray-700">{{ number_format($product->favorites_count) }}</span>
                            </div>
                        </div>
                        <div class="p-4">
                            <span class="inline-block px-3 py-1 bg-[#fff7ed] text-[#f97316] text-xs font-semibold rounded-full mb-2">{{ $product->category ?? 'Menu' }}</span>
                            ...
                            <div class="flex items-center justify-between">
                                <span class="text-xl font-bold text-[#f97316]">Rp. {{ number_format($product->price) }}</span>
                                <div class="flex items-center gap-1">
                                    <span class="text-yellow-400">⭐</span>
                                    <span class="text-sm font-medium text-gray-600">{{ number_format($product->rating ?? 5, 1) }}</span>
                                </div>
                            </div>
                        </div>
                    </a>
