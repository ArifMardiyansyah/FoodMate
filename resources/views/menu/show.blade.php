...
<head>
    ...
    <title>{{ $product->name }} - FoodMate</title>
    ...
    <div class="max-w-4xl mx-auto px-6 py-8 pb-24">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="/cart" class="relative inline-flex items-center justify-center w-12 h-12 bg-[#fff7ed] rounded-full hover:bg-[#ffedd5] transition">
                ...
                <span id="detailCartCount" class="absolute -top-1 -right-1 min-w-[20px] h-5 bg-[#f97316] rounded-full text-xs font-semibold text-white flex items-center justify-center px-1">{{ number_format($cartCount) }}</span>
            </a>
        </div>

        <div class="relative mb-6">
            <div class="w-full h-64 bg-[#f97316] rounded-3xl flex items-center justify-center overflow-hidden shadow-xl">
                <img src="{{ asset($product->image_path ?? 'images/food/default.png') }}" alt="{{ $product->name }}" class="w-full h-full object-contain">
            </div>
        </div>

        <div class="mb-6">
            <span class="inline-block px-4 py-1.5 bg-[#fff7ed] text-[#f97316] text-sm font-semibold rounded-full mb-3">{{ $product->category ?? 'Menu' }}</span>
            ...
            <div class="flex items-center gap-6 mb-4 text-gray-500">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-medium">{{ number_format($product->rating ?? 5, 1) }} Rating</span>
                </div>
                <div class="flex items-center gap-2">
            ...
            <p class="text-gray-700 leading-relaxed mb-6">{{ $product->description ?? 'Menu favorit pelanggan FoodMate.' }}</p>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-lg border border-gray-100 p-6 mb-6">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-3xl font-bold text-[#f97316]">Rp. {{ number_format($product->price) }}</p>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p id="favoritesCount" class="text-lg font-semibold text-gray-700">
                        <span id="favoritesNumber">{{ number_format($favoritesCount) }}</span> orang menyukai menu ini
                    </p>
                </div>
                <button id="favoriteButton" class="w-14 h-14 rounded-full flex items-center justify-center transition-all duration-300 {{ $isFavorited ? 'bg-red-50' : 'bg-gray-100' }} hover:scale-110">
                    <svg id="heartIcon" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="{{ $isFavorited ? '#ef4444' : 'none' }}" stroke="{{ $isFavorited ? '#ef4444' : '#6b7280' }}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="transition-all duration-300">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @foreach($recommended as $item)
                        <a href="{{ route('menu.show', $item->slug) }}" class="flex gap-4 p-4 border border-gray-100 rounded-2xl hover:shadow-lg transition-shadow">
                            <div class="w-20 h-20 rounded-xl overflow-hidden bg-gray-100 flex-shrink-0">
                                <img src="{{ asset($item->image_path ?? 'images/food/default.png') }}" alt="{{ $item->name }}" class="w-full h-full object-cover">
                            </div>
                            <div class="flex-1">
                                <h3 class="text-lg font-bold text-black mb-1">{{ $item->name }}</h3>
                                <p class="text-sm text-gray-500 line-clamp-2">{{ $item->description }}</p>
                                <div class="flex items-center justify-between mt-2">
                                    <span class="text-[#f97316] font-semibold">Rp. {{ number_format($item->price) }}</span>
                                    <span class="text-xs text-gray-400">⭐ {{ number_format($item->rating ?? 5, 1) }}</span>
                                </div>
                            </div>
                        </a>
                    @endforeach
