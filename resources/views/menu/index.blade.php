            ...
            <div class="max-w-7xl mx-auto flex justify-between items-center h-20">
                ...
                <div class="flex items-center gap-6">
                    <a href="/cart" class="relative inline-flex items-center justify-center w-12 h-12 rounded-full bg-[#F6A406]/10 hover:bg-[#F6A406]/20 transition">
                        ...
                        <span id="menuCartCount" class="absolute -top-1 -right-1 min-w-[22px] h-[22px] rounded-full bg-[#F6A406] text-white text-xs font-semibold flex items-center justify-center px-1">{{ number_format($cartCount) }}</span>
                    </a>
    ...
    <section class="py-16 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid gap-8 md:grid-cols-3">
                ...
                <div class="bg-[#F6A406] rounded-xl p-6 text-center">
                    <div class="w-12 h-12 bg-[#F6A406] rounded-full flex items-center justify-center mx-auto mb-4">
                        <img src="{{ asset('images/icon/rush (1).png') }}" alt="Cepat" class="w-12 h-12">
                    </div>
                ...
                <div class="bg-[#F6A406] rounded-xl p-6 text-center">
                    <div class="w-12 h-12 bg-[#F6A406] rounded-full flex items-center justify-center mx-auto mb-4">
                        <img src="{{ asset('images/icon/fresh (1).png') }}" alt="Fresh" class="w-12 h-12">
                    </div>
    ...
    <section class="bg-white px-6 lg:px-12 py-16">
        <div class="max-w-7xl mx-auto">
            ...
            <div id="menuSections" class="space-y-16">
                ...
                @endphp
                @foreach($sorted as $category => $products)
                    <div class="space-y-6" data-category-section>
                        <div class="flex items-center justify-between">
                            <h3 class="text-3xl font-bold text-black">{{ $category }}</h3>
                            <span class="text-sm text-gray-400">{{ $products->count() }} pilihan</span>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($products as $product)
                                <div class="bg-white rounded-2xl p-4 shadow-lg" data-menu-card data-name="{{ strtolower($product->name) }}" data-category="{{ strtolower($category) }}">
                                    <a href="{{ route('menu.show', $product->slug) }}" class="block">
                                        <div class="w-90 h-70 bg-gray-100 rounded-xl mb-4 overflow-hidden">
                                            <img src="{{ asset($product->image_path ?? 'images/food/default.png') }}"
                                                 alt="{{ $product->name }}"
                                        ...
                                        <div class="flex items-center mb-2">
                                            ...
                                            <span class="text-sm font-bold">{{ number_format($product->rating ?? 5, 1) }}</span>
                                    ...
                                    @endphp
                                    <button class="w-full bg-orange-500 text-white py-3 rounded-xl font-bold hover:bg-orange-600 transition" data-action="add" data-product='@json($productPayload)'>Pesan</button>
                                </div>
                            @endforeach
                        </div>
                    </div>
